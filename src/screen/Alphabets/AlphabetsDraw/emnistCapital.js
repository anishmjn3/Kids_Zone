...
export default function EminstCapitalMain() {
  ...
  useEffect(() => {
    GetPermission()
    onSelectModel('model')
    Tts.speak('Lets Draw Capital Alphabets');
        ...
        tflite.runModelOnImage({
          ...
          (err, res) => {
            if (err)
              ...
              if (strokeColor) {
                if (res.length > 0)
                  putImageToFirebase(`capitalalphabets/${res[0].label}/${Math.floor(date.getTime() + date.getSeconds() / 2)}.png`, file_path)
                else
                  putImageToFirebase(`capitalalphabets/${Math.floor(date.getTime() + date.getSeconds() / 2)}.png`, file_path)
