...
export default function EminstSmallMain() {
  ...
  useEffect(() => {
    GetPermission()
    onSelectModel('model')
    Tts.speak('Lets Draw Small Alphabets');
  ...
  const onSelectModel = (model) => {
    // this.setState({ model });
    setModel(model)
    var modelFile = "models/emnistSmall.lite";
    var labelsFile = "models/emnistSmall.txt";
    tflite.loadModel({
      model: modelFile,
      labels: labelsFile,
    },
        ...
        tflite.runModelOnImage({
          ...
          (err, res) => {
            if (err)
              ...
              if (strokeColor) {
                if (res.length > 0)
                  putImageToFirebase(`smallalphabets/${res[0].label}/${Math.floor(date.getTime() + date.getSeconds() / 2)}.png`, file_path)
                else
                  putImageToFirebase(`smallalphabets/${Math.floor(date.getTime() + date.getSeconds() / 2)}.png`, file_path)
                if (smallalphabet[NumberValue] == res[0].label)
                  ding()
                else
                  buzzer()
              }
    ...
    <View style={{ backgroundColor: "#fff", height: height, width: width }}>
      <View style={[{ padding: height * 0.025, flexDirection: 'row', backgroundColor: '#054014', width: width }, styles.centerAlign]}>
        <Text style={{ fontSize: height * 0.023, fontWeight: 'bold', color: '#fff' }}>
          Lets Draw Small Alphabets
        </Text>
      </View>

      <View style={[{ width: width, alignItems: 'center' }]}>
        <View style={[{ height: height * 0.12, },]}>
          <Text style={{ fontSize: height*0.06 }}>
            {smallalphabet[NumberValue]}
          </Text>
        </View>
      </View>
      <View
